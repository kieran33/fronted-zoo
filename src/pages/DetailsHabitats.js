...
const DetailsHabitats = () => {
  ...
  const loadData = async () => {
    const reponse = await axios.get(
      "https://backend-zoo-production.up.railway.app/habitats"
    );
  ...
  const loadDataAnimaux = async () => {
    const reponse = await axios.get(
      "https://backend-zoo-production.up.railway.app/animaux"
    );
    ...
    <>
      ...
      <div className="centrer">
        <div className="animal">
          ...
          <img
            className="image_zoo_details"
            src={`https://backend-zoo-production.up.railway.app/image/${dataHabitat.image}`}
            alt={dataHabitat.nom}
          ></img>
          <p className="paragraphe" style={{ marginTop: "50px" }}>
            {dataHabitat.description}
          </p>
          {dataHabitat.etat === "" ? (
            <></>
          ) : (
            <div>
              <h3 className="titre_service">
                L'avis du vétérinaire sur l'état de l'habitat
              </h3>
              <p style={{ textAlign: "center" }}>{dataHabitat.etat}</p>
            </div>
          )}
          <h3 className="titre_service">
            Liste des animaux qui vivent dans cet habitat
          </h3>
          <div className="centrer">
            {dataAnimaux
              .filter((animal) => animal.habitat === dataHabitat.nom)
              .map((animal, index) => (
                <div className="animal" key={index}>
                  <div className="div_zoo_animaux">
                    <Link
                      to={`/animaux/${animal.id}/${animal.prenom}`}
                      style={{ opacity: "1" }}
                    >
                      <img
                        className="image_zoo_animaux"
          ...
          <div className="centrer">
            {data
              .filter((habitat) => habitat.nom !== dataHabitat.nom)
              .map((habitat, index) => (
                <div className="animal" key={index}>
                  <div className="div_zoo_animaux">
                    <Link
